...
# cbind into one table
ORs_CIs_young_median=cbind(foo(lung_res.1),foo(lung_res.2, smoking = T),
                           foo(bladder_res.1),foo(bladder_res.2, smoking = T))
tmp=as.data.frame(model.matrix(~.,lung_data[,-1])[,-1])
mynames=colnames(tmp)
rownames(ORs_CIs_young_median)=mynames[-c(1:7)]
ORs_CIs_young_median <- as.data.frame(ORs_CIs_young_median)

### P vals
pvals_young_median=cbind(do.call("rbind", lapply(lung_res.1, `[[`, 2)),
                         c(NA, do.call("rbind", lapply(lung_res.2, `[[`, 2))),
                         do.call("rbind", lapply(bladder_res.1, `[[`, 2)),
                         c(NA, do.call("rbind", lapply(bladder_res.2, `[[`, 2))))
# Change to log scale
pvals_young_median=-log10(pvals_young_median)
# Change column name
colnames(pvals_young_median)=paste0(rep(c("lung","bladder"),each=2),".",1:2)
pvals_young_median <- as.data.frame(pvals_young_median)
...
# cbind into one table
ORs_CIs_old_median=cbind(foo(lung_res.1),foo(lung_res.2, smoking = T),
                          foo(bladder_res.1),foo(bladder_res.2, smoking = T))
tmp=as.data.frame(model.matrix(~.,lung_data[,-1])[,-1])
mynames=colnames(tmp)
rownames(ORs_CIs_old_median)=mynames[-c(1:7)]
ORs_CIs_old_median <- as.data.frame(ORs_CIs_old_median)

### P vals
pvals_old_median=cbind(do.call("rbind", lapply(lung_res.1, `[[`, 2)),
                        c(NA, do.call("rbind", lapply(lung_res.2, `[[`, 2))),
                        do.call("rbind", lapply(bladder_res.1, `[[`, 2)),
                        c(NA, do.call("rbind", lapply(bladder_res.2, `[[`, 2))))
# Change to log scale
pvals_old_median=-log10(pvals_old_median)
# Change column name
colnames(pvals_old_median)=paste0(rep(c("lung","bladder"),each=2),".",1:2)
pvals_old_median <- as.data.frame(pvals_old_median)
...
colnames(ORs_CIs_young_median) <- paste0('young_median_', colnames(ORs_CIs_young_median))
colnames(ORs_CIs_old_median) <- paste0('old_median_', colnames(ORs_CIs_old_median))

#Then join
ORs_age_median <- cbind(ORs_CIs_young_median, ORs_CIs_old_median)

dir.create("../Results/sensitivity")
saveRDS(ORs_age_median, "../Results/sensitivity/ageatdiag_median_ORs.rds")


#change column names
colnames(pvals_young_median) <- paste0('young_median_', colnames(pvals_young_median))
colnames(pvals_old_median) <- paste0('old_median_', colnames(pvals_old_median))

#Then join
pvals_age_median <- cbind(pvals_young_median, pvals_old_median)

saveRDS(pvals_age_median, "../Results/sensitivity/ageatdiag_median_pvals.rds")



#change column names
colnames(ORs_CIs_early_median) <- paste0('early_median_', colnames(ORs_CIs_early_median))
colnames(ORs_CIs_late_median) <- paste0('late_median_', colnames(ORs_CIs_late_median))

#Then join
ORs_time_median <- cbind(ORs_CIs_early_median, ORs_CIs_late_median)

saveRDS(ORs_time_median, "../Results/sensitivity/timetodiag_median_ORs.rds")


#change column names
colnames(pvals_early_median) <- paste0('early_median_', colnames(pvals_early_median))
colnames(pvals_late_median) <- paste0('late_median_', colnames(pvals_late_median))

#Then join
pvals_time_median <- cbind(pvals_early_median, pvals_late_median)

saveRDS(pvals_time_median, "../Results/sensitivity/timetodiag_median_pvals.rds")