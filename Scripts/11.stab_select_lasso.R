...
## Programme created by Rin Wada on 23 March
...
out=CalibrateRegression(xdata=x_train, ydata=y_train, K=100, tau=0.5, verbose=FALSE, family="binomial")
t1=Sys.time()
print(t1-t0)
...
saveRDS(myaucs_table, paste0("../Results/lasso/auc_average_beta_",arr,".rds"))
...
for (k in 1:length(myorder)){
  ...
  myroc=roc(response=y_test, predictor=mymodel$fitted.values)
  if(k==calib){
    saveRDS(myroc, paste0("../Results/lasso/roc_recalib_beta_",arr,".rds"))
  }
  myaucs=c(myaucs, myroc$auc)
...
rownames(myaucs_table)=myorder
colnames(myaucs_table)=c("li","auc","ui")
saveRDS(myaucs_table, paste0("../Results/lasso/auc_recalib_beta_",arr,".rds"))
...
# }
# saveRDS(mr_controls, paste0("../Results/lasso/mr_controls_",arr,".rds"))
# saveRDS(mr_cases, paste0("../Results/lasso/mr_cases_",arr,".rds"))
