...
var_label(mydata) <- list(age_baseline.x='Age', sex='Sex', ethnic='Ethnicity',
                          ...
                          own_rent='Own or rent accomodation lived in', num_hh='Number of people in ousehold',
                          ...
                          mat_smoke='Maternal smoking around birth', no2='NO2 (??g/m3)',
                          nox='NOx (??g/m3)', pm10='PM10 (??g/m3)', pm2.5_absorb='PM2.5 (absorbance/m)',
                          pm2.5='PM2.5 (??g/m3)', pm2.5_10='PM2.5-10??m (??g/m3)',
...
t0 <- mydata %>%
  ...
  tbl_summary(by = case_status, missing = "no",               
              ...
              value = list(all_of(dichotomousVars) ~ "level to show")) %>%
  modify_header(update = list(stat_1 ~ '**Control (n=449042)**',
                stat_2 ~ '**Lung (n=1987)**',
                stat_3 ~ '**Bladder (n=1724)**')) %>%
  add_n() %>%
  bold_labels()
