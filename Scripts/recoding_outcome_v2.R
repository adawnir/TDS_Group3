### TDS Project -- Recoding outcome NEW
...
### If a participant was diagnosed with lung/bladder cancer before baseline
### OR self-reported lung or bladder cancer at baseline, they were excluded.

# Self-reported codes
self_case=as.character(c(1001,1027,1028,1080, # lung
                         1035)) # bladder
self_eid=apply(outcome[,paste0("self",1:6)],2,
               function(x) outcome$eid[grepl(paste0(self_case,collapse="|"), x)]) %>% unlist()
# Remove
def_pop=outcome[-which(outcome$date_baseline >= outcome$epistart_lung |
                        outcome$date_baseline >= outcome$epistart_bladder |
                         outcome$eid %in% self_eid),]
...
def_pop2=def_pop2[-which((!is.na(def_pop2$epistart_lung)|!is.na(def_pop2$epistart_bladder)) &
                           def_pop2$eid %in% self_any),]
nrow(def_pop)-nrow(def_pop2)

# Check for co-occurring lung and bladder cancer
tmp=def_pop2[which(!is.na(def_pop2$epistart_bladder)&!is.na(def_pop2$epistart_lung)),] # 18 co-occurring cases
# Remove diagnosed on same day
def_pop2=def_pop2[-which(def_pop2$epistart_lung==def_pop2$epistart_bladder),]
nrow(def_pop)-nrow(def_pop2)
# Only keep first diagnosis
def_pop2[which(def_pop2$epistart_bladder>def_pop2$epistart_lung),
         c("diag_icd9_lung","diag_icd10_lung","epistart_lung")]=NA
def_pop2[which(def_pop2$epistart_lung>def_pop2$epistart_bladder),
         c("diag_icd9_bladder","diag_icd10_bladder","epistart_bladder")]=NA

## Case/control status
def_pop2$case_status=0
def_pop2$case_status[!is.na(def_pop2$epistart_bladder)]=2
def_pop2$case_status[!is.na(def_pop2$epistart_lung)]=1
table(def_pop2$case_status)
...
def_pop3$time_diag_days=difftime(def_pop3$epistart,
                                 def_pop3$date_baseline,
                                 units = "days") %>% as.numeric()
def_pop3$time_diag_years=def_pop3$time_diag_days/365.25
def_pop3$age_diag=def_pop3$age_baseline+def_pop3$time_diag_years
### Restructure ---
case_control=def_pop3 %>% select(eid:age_baseline,case_status:age_diag) %>%
  mutate(case_status=factor(case_status,levels = c(0:2),labels = c("control","lung","bladder")))
str(case_control)
table(case_control$case_status)

saveRDS(case_control, "../Results/case_control_alt.rds")
