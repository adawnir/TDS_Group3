...
colnames=names(lasso_selprop_lung.nvsm.1)
myorder=c(colnames[5:46],colnames[1:4],colnames[47:length(colnames)])

# gather data
lasso_selprop_nvsm=foo(lasso_selprop_lung.nvsm.1,
                         lasso_selprop_bladder.nvsm.1,
                         myorder)
spls_selprop_nvsm=foo(spls_selprop_lung.nvsm.1,
                        spls_selprop_bladder.nvsm.1,
                        myorder)
beta_nvsm=foo2(lasso_beta_lung.nvsm.1, lasso_beta_bladder.nvsm.1,
                 spls_beta_lung.nvsm.1,spls_beta_bladder.nvsm.1,myorder)
...
## nvsm
pi_spls=c(spls_hat_params_lung.nvsm.1[2],spls_hat_params_bladder.nvsm.1[2]) # sPLS Selection proportion threshold
pi_lasso=c(lasso_hat_params_lung.nvsm.1[2],lasso_hat_params_bladder.nvsm.1[2]) # LASSO Selection proportion threshold
file_path="nvsm"
...
### Beta plot----
variable_cat=c(rep("Sociodemographic",14),
               rep("Health risk", 28),
               rep("Environmental", 8),
               rep("Medical", 16), 
               rep("Biomarkers", 28))
...
beta_nvsm$label=rep(1:(nrow(beta_nvsm)/2), 2)
beta_nvsm$mycolour_point=c(lighten(rep(mycolours,times=c(14,28,8,16,28)),0.2),rep(mycolours,times=c(14,28,8,16,28)))
