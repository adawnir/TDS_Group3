...
## Programme created by Rin Wada on 9 March
...
if (grepl("0$",arr)){
  ...
  pdf(paste0("../Figures/lasso/out_",arr,".1",".pdf"), height=5, width=10)
  CalibrationPlot(out.1)
  dev.off()
  pdf(paste0("../Figures/lasso/out_",arr,".2",".pdf"), height=5, width=10)
  ...
  selprop_nonzero.1=selprop_nonzero.1[-which(names(selprop_nonzero.1) %in%
                                               colnames(x_train)[1:7])]
  ...
  for (k in 1:length(myorder)){
    ...
    mymodel=glm(y_test~offset(x_test[,c(colnames(x_test)[1:7],myorder[1:k]),drop=FALSE]%*%
                                matrix(average_beta[c(colnames(x_test)[1:7],myorder[1:k])],
                                       ncol=1)), family="binomial")
  ...
  selprop_nonzero.2=selprop_nonzero.2[-which(names(selprop_nonzero.2) %in%
                                               colnames(x_train)[1:11])]
  myorder=names(selprop_nonzero.2)[sort.list(selprop_nonzero.2, decreasing = TRUE)]
  myaucs=NULL
  myaucs_table=NULL
  for (k in 1:length(myorder)){
    # Using the average beta as a beta, only computing the intercept in the model below (no impact on AUC)
    mymodel=glm(y_test~offset(x_test[,c(colnames(x_test)[1:11],myorder[1:k]),drop=FALSE]%*%
                                matrix(average_beta[c(colnames(x_test)[1:11],myorder[1:k])],
                                       ncol=1)), family="binomial")
    myroc=roc(response=y_test, predictor=mymodel$fitted.values)
    myaucs=c(myaucs, myroc$auc)
    myaucs_table=rbind(myaucs_table, formatC(as.numeric(ci.auc(myroc)), format="f",
  ...
  out=CalibrateRegression(xdata=x_train, ydata=y_train, K=100, tau=0.5, verbose=FALSE,
                          family="binomial")
  # Save plot
  pdf(paste0("../Figures/lasso/out_",arr,".pdf"), height=5, width=10)
  CalibrationPlot(out)
  dev.off()
